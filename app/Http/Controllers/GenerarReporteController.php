...
{
    public function generarReporte(PozosIdRequest $request)
    {
    ...
    ";
        ...
        foreach ($pozosIDs as $pozoId) {
            // binding para evitar inyección y para enviar parámetros (IdPozo, Fecha)
            $consulta = DB::select($sql, [$pozoId, $fecha]);
            if (! empty($consulta)) {
                $ReportePozo = array_merge($ReportePozo, [
                    $consulta[0]->Pozo => $consulta,
                ]);
            }
        }
